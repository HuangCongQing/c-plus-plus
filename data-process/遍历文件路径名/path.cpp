 ...
 * @LastEditTime: 2021-04-15 16:16:59
...
int main()
{
    ...
    for (auto &it : boost::filesystem::directory_iterator(pcd_all_path))
    {
        ...
        std::string filename = it.path().filename().string(); //遍历出来的文件名称
        std::string extension = it.path().extension().string(); //遍历出来的文件名称

        std::cout << "strPath:" << strPath << std::endl;
        std::cout <<  " filename:" << filename << std::endl;  // filename:1600050937.416284.pcd
